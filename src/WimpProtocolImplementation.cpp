...
#define MAX_PATH_LENGTH (64 * 1024) - 1
...
uint16_t path_length = MAX_PATH_LENGTH; 
...
Serial.printf("IP: %s\n PATH: %d\n", other, other_path);
#endif
            children[i].len_path = std::min(other_path, path_length+1);
...
Serial.printf("IP: %s\n PATH: %d\nNum_neighbours: %d\n", other, other_path, num_neighbours);
#endif
            }
        }
    }
...
Serial.printf("DATA: %s\n", data);
        ...
        IPAddress next_hop;
        next_hop.fromString((char*) ip_path[0]);

        Serial.println("Next hop: %s", next_hop);
...
for (int i=0; i<num_neighbours; ++i) {
    Serial.printf("%s \t %s \t %d \t %d \t %d\n", neighbours[i].ip, neighbours[i].ssid, neighbours[i].rssi, neighbours[i].len_path, neighbours[i].times_not_seen);
}
#endif
...
Serial.printf("Connected to: %s\n", neighbours[i].ssid);
            ...
            if (change_parent(neighbours[i].ip, my_ip)) {
                ...
                hello(parent.ip, "hello");  //prendo info sul path
                connected = true;
#if debug
...
*/
void hello(IPAddress dest, char* type) {  
    char msg[LEN_PACKET];
    JsonObject& ans = json_buffer.createObject();
...
bool change_parent(IPAddress new_parent, IPAddress old_parent) {
    ...
    root["handle"] = "change";
    root["ip_source"] = my_ip;
    root["ip_old_parent"] = old_parent;
